...
DATASET_ROOT = "/data/post_intern_sahithya/ariav5/projectaria_tools_ase_data/" 
#TODO: this needs to be a commandline param
SCENE_ID = 1 
...
for frame_idx in range(0, num_frames, 10):
    ...
    # Convert instance IDs to unique colors
...
min_x, min_y = x.min(), y.min()
xi = ((x - min_x) / 0.01).astype(int)
yi = ((y - min_y) / 0.01).astype(int)
...
for inst_id in unique_ids:
    ...
    # Get class color and convert to BGR and 0-255
...
# Get all camera positions in world?? coordinates
camera_positions = []
for T in trajectory["Ts_world_from_device"]:
    # T - 4x4 matrix or SE3(?), get translation part : TODO: cross-check this conversaion
    if hasattr(T, "translation"):  # Random check TODO: This is hacky
        t = T.translation()
    else: 
        t = T[:3, 3]
...
cam_x, cam_y, cam_z = camera_positions[:, 0], camera_positions[:, 1], camera_positions[:, 2]


cam_xi = ((cam_x - min_x) / 0.01).astype(int)
cam_yi = ((cam_y - min_y) / 0.01).astype(int)
cam_img_x = cam_xi
cam_img_y = H - cam_yi - 1

#Plot cam traj #TODO: this needs to be a commandline param
plt.figure(figsize=(10, 10))
plt.imshow(rgb_img_outline)
plt.plot(cam_img_x, cam_img_y, color='cyan', linewidth=2, label='Camera Trajectory')
plt.scatter(cam_img_x, cam_img_y, color='red', s=10, label='Camera Positions')
plt.title("Top-Down RGB Map with Instance Outlines, Labels, and Camera Trajectory")
plt.axis("off")
...
# Orientation arrows #TODO: this needs to be a commandline param
arrow_scale = 20  # Adjust for visibility
for i, T in enumerate(trajectory["Ts_world_from_device"]):
    if hasattr(T, "rotation"):
        ...
        Rmat = T[:3, :3]
        t = T[:3, 3]
    forward = Rmat[:, 2]  # or -Rmat[:, 2] #TODO: is this correct?
    x_img = int((t[0] - min_x) / 0.01)
    y_img = H - int((t[1] - min_y) / 0.01) - 1
...
plt.tight_layout()
plt.savefig("topdown_rgb.png", dpi=300, bbox_inches='tight')
plt.show()
